...
describe('StatusRemark discovery count', () => {
  test('renders the discovery count as a compact badge when a service has historical discoveries', () => {
    const service = makeService({ newVersionDiscoveryCount: 123 })
    ...
    expect(html).toContain('statusLineLabelAnchor')
    expect(html).toContain('statusLineLabelText')
    expect(html).toContain('discoveryHistoryTriggerGroupCompact')
    expect(html).toContain('discoveryHistoryTriggerCompact')
    expect(html).toContain('aria-label="发现 123 次，打开 GitHub Releases 抽屉"')
    expect(html).toContain('aria-label="发现 123 次，查看版本时间线"')
    expect(html).toContain('>123</button>')
    expect(html).not.toContain('>发现 123 次</button>')
