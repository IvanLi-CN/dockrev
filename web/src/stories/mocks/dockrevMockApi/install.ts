import type {
  ...
  ServiceLifecycleStatusResponse,
...
export function installDockrevMockApi(
  ...
  globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (method === 'GET' && urlPath.startsWith('/api/services/') && urlPath.endsWith('/lifecycle-status')) {
      ...
      const stateByScenario: Record<string, ServiceLifecycleStatusResponse> = {
      ...
      return json(stateByScenario[scenario] ?? { state: 'stopped' } satisfies ServiceLifecycleStatusResponse)
    }
    if (method === 'POST' && urlPath.startsWith('/api/services/') && urlPath.endsWith('/lifecycle')) {
      const serviceId = decodeURIComponent(urlPath.split('/').slice(3, -1).join('/'))
      const found = findService(serviceId)
      if (!found) return json({ error: { code: 'not_found', message: 'service not found' } }, { status: 404 })
      ...
      f.jobs = [job, ...f.jobs]
      f.jobById[jobId] = { ...job, logs: [{ ts: createdAt, level: 'info', msg: `Service lifecycle ${action} started.` }], logsLastId: 1 }
      return json({ jobId })
    }
