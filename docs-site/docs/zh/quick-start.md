...
Dockrev 会稳定保持 Web App Manifest 的 `id`、`scope` 和 `start_url`，不会因为图标版本变化而改变已安装应用的身份。每个安装图标都使用内容哈希文件名发布，HTML、manifest 和 service worker 会重新验证；新构建可以因此交付新的图标字节，不需要把重新安装作为常规更新方式。
