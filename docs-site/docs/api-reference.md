...
- 服务生命周期契约：
  ...
  - `POST /api/services/{service_id}/lifecycle` 请求体为 `{ "action": "start" | "stop" | "restart" }`。启动显式执行 `up -d --pull never`；Compose CLI 无法接受该约束时任务失败，不会回退到可能拉取镜像的路径。停止和重启分别执行对应 Compose 命令。
