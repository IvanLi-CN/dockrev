...
- `CI (main)` materializes immutable release snapshots into git notes `refs/notes/release-snapshots`; normal release-enabled pushes may backfill missing first-parent snapshots, while `type:skip` infrastructure pushes only materialize their own disabled target.
...
- Candidate `operation=verify` is non-publishing. A separately authorized `operation=recover` requires an exact main SHA and non-empty audit reason, accepts only the oldest unreleased first-parent target, and writes only a `candidate-recovery` snapshot plus an audited readiness entry.
- The `Release` workflow consumes only readiness receipts for the oldest pending target. It never polls another workflow, bypasses an older missing receipt, or dispatches automatic recovery; a missing or mismatched receipt fails closed.
- Before selecting the oldest pending snapshot with a valid receipt, the `Release` workflow reconciles any older tag-backed targets that already have a GitHub Release plus resolvable GHCR digests, backfilling their missing `refs/notes/release-publications` entries instead of letting historical ledger gaps block newer releases forever.
- Recovery treats tags from before the trusted publication anchor as legacy evidence only when no immutable release snapshot exists; it does not reconstruct their intent from mutable current PR labels. Snapshot-backed release-enabled tags and post-anchor tag-only gaps still block FIFO.
...
- That explicit `workflow_dispatch(..., admin_action=release)` path still requires the requested SHA to be the oldest pending target with a valid readiness receipt; it cannot bypass an older missing or invalid receipt.
...
- `Release Candidate Pipeline` verification runs the candidate gates without writing snapshots/readiness notes or creating tags, GHCR images, or GitHub Releases; recovery and publication remain separate owner-confirmed actions.
