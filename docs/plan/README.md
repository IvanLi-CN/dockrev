...
| 0006 | CI/CD: 自动发版意图标签与发布限制（防止 docs-only 发版） | 部分完成（1/2） | `0006:release-intent-label-gating/PLAN.md` | 2026-01-22 | 参考 catnap PR #9：PR label gate（type:docs|skip|patch|minor|major，缺失即失败）+ main release-intent；无关联 PR / API 失败 / 多 PR=保守跳过；bump 仅由标签决定（major|minor|patch）；安全：label gate 不得执行 PR checkout 的脚本（需修复） |
