...
- 通用：
  - busy（已有请求进行中）时禁用。
  - 若 UI 无法确定“是否可更新”，也应允许触发，但必须在确认文案中提示“将由服务端按候选版本/忽略规则计算是否实际变更”。
- service scope：
  - 当该 service 在 UI 中呈现为“可更新（updatable）”时启用；否则禁用并显示原因（无候选/架构不匹配/被阻止）。
- stack / all scope：
  - 当目标范围内至少存在一个“可更新（updatable）”服务时启用；否则禁用并显示“无可更新服务”。
- Dockrev 自身：
