...
- 不新增独立导航 API；继续复用现有 stack/service 响应。
- 不改动服务详情、任务队列、更新执行语义。
...
- Integration tests: stack/service API round-trip for Homepage metadata；页面搜索/新标签页行为的 Storybook `play` 覆盖
- E2E tests (if applicable): Storybook-driven interaction checks for overview navigation and services candidate search
...
- source_type: storybook_canvas
  ...
  state: `operations dashboard default`
  evidence_note: 验证 `/services` 已接管运维大盘，并在下方保留 archived 恢复区。

  ![运维大盘页面](./assets/services-operations-dashboard.png)
...
- [x] M5: Storybook、视觉证据、浏览器 smoke、review-loop 与 PR-ready 收口完成
...
- 风险：页面职责互换后，旧的 Storybook 场景、文案和路由断言容易一起失效，需要整体刷新。
