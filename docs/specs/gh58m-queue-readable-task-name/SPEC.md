...
- Frontend display helpers（新增）：
  ...
  - `formatJobReadableDisplay(type: string, scope: string): { primaryLabel; scopeTag; typeTone }`
...
- Given 队列中存在不同 `job.type`（如 `update`/`check`/`runtime_scan`/`discovery`），When 渲染任务名标签，Then 不同任务类型使用可区分的 badge 颜色。
