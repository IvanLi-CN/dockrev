...
- 服务详情通过页面状态把当前服务名与资源摘要渲染到 AppShell 顶栏：摘要位于名称和操作组之间，正文不再渲染重复的服务标题或资源摘要。摘要使用容器宽度控制三个不可拆分指标组的可见性，按网络、磁盘、CPU/内存顺序渐进隐藏。移动端资源摘要整体隐藏，页头保持单行，以图标 Logo、当前服务名和 44px 服务操作入口构成；入口使用 shadcn/Radix DropdownMenu，将更新、生命周期与 Stack 三组动作直接平铺并以库内分隔线区分。
