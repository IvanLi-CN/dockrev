...
- `service_lifecycle` 与同一服务的 apply update、rollback 采用同一冲突保护；任务占锁与首条日志在同一 SQLite 事务提交，避免日志失败留下永久活动锁；定向 Stack/全局更新按持久化的实际服务目标占锁，仅对缺少目标记录的旧活动任务回退到 scope 判断；任务摘要、队列显示与服务操作历史均保留并展示具体动作。
