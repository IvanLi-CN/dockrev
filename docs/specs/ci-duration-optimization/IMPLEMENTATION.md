...
- For each run, capture the run id from `gh workflow run`, then poll `gh run view <run-id> --json status,conclusion` every 15 seconds until the workflow reaches a natural terminal state. The observer keeps a fixed 180-second grace after the 720-second execution threshold so reusable workflow outputs and metrics can settle; it never cancels a run at the execution boundary. Metrics still reject execution over 720 seconds (and warm execution over 600 seconds), so the grace cannot make a slow sample pass. A terminal failure, observation timeout, missing artifact, SHA mismatch, or failed child gate consumes that sample and is never retried automatically.
