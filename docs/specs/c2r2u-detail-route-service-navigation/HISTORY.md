...
- 2026-08-01：移动服务抽屉移除箭头所指的抽屉头重复标题，保留树内容区域标题，并让服务树列表填充剩余高度、独立承载溢出滚动。
- 2026-09-04：修正移动端品牌规则误把 Stack 详情裁切为图标的问题；Shell 现在按详情路由类型隔离品牌样式，Stack 维持完整字标，Service 维持紧凑图标，并补齐 320px / 960px 真实视口回归。
- 详情服务树移除与抽屉标题重复的树标题和当前路径；搜索独占一行，Stack/服务统计位于下一行右侧。响应式树在移动断点挂载 mobile 样式，保证小屏幕中的输入区域可用。
- 最近扫描改为服务树摘要行左侧的相对时间，与 Stack/服务统计共享第二行，不再在抽屉 footer 前占用独立行。
- 移动抽屉 footer 将身份入口与单个主题循环按钮整合为一行，去除三段式主题选择器的独占高度。
