...
- 已让 `AppShell` 显式区分 Stack 与 Service 详情页类型；移动端 Stack 保持完整 124px Dockrev 字标，Service 保持 36px 图标标记，并由真实 320px / 960px Playwright 视口断言单行与无横向溢出。
...
- 服务树工具栏只保留搜索与摘要信息，移除重复的树标题和当前路径；搜索独占第一行，移动端的最近扫描以人可读相对时间显示在第二行左侧，Stack/服务统计保持右对齐。扫描超过 7 天会改为琥珀色“扫描过期”提示，避免将陈旧数据误读为新鲜状态。移动抽屉在断点下实际挂载 mobile tree 样式，列表参与抽屉剩余高度分配并在内容过长时独立滚动。
- 移动抽屉 footer 将身份入口与单个主题循环按钮并列，移除三段式主题选择器，保留右键或键盘主题菜单。
