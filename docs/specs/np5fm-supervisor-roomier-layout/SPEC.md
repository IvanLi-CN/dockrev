...
- 2026-03-11: 同步 main 解决 `docs/specs/README.md` 索引冲突，并修复 review-loop 指出的两处复制反馈问题：`previous.tag=unknown` 不再作为可复制/可回滚目标，`execCommand('copy')` fallback 在失败时会正确进入错误态。
- 2026-03-11: 修正离线缓存态的 `Previous` 展示：无可回滚目标时不再渲染 `- · stale` 这类伪有效值，而是回落为离线占位文案，并与复制按钮禁用状态保持一致。
- 2026-03-12: 进一步校正复制语义：当 Clipboard API 在运行时被拒绝时，会自动退回 `execCommand('copy')` fallback；`Previous` 的复制值改为与后端 rollback 一致的真实镜像引用，不再复制仅供展示的 tag/digest 文本。
