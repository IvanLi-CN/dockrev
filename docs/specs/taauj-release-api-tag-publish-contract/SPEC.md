...
- `#yt22e` 已经把 release queue 的误标 target 与 skip override 收口好了。
...
- 目标不是增加额外凭据，而是在现有 GitHub 原生权限模型内，让 release 继续发布 GitHub Release，并保证 publication ledger 只在 GitHub Release 成功后才记为完成。
...
- 让 publication ledger 只在 GitHub Release 成功后记录，避免发布失败时 queue 错误前进。
- successful publication 由 release-owning agent 报告给 owner；workflow 不向 source PR 写入发布结果或状态评论。
- 不引入任何额外凭据。
...
- `.github/scripts/release_pr_comment.py`（删除）
...
- publication ledger 只能在 GitHub Release 成功后记录。
- successful publication 不以 source PR 发布结果或状态评论为前置条件。
...
- `workflow_dispatch(..., admin_action=release)` 仍按精确 SHA 语义工作：先确保该 SHA 的 immutable snapshot 与 readiness receipt 存在，再按 FIFO 校验不得绕过更老目标。
...
- GitHub Release 成功后记录 publication ledger；只有此时 target 才算 published，可继续 queue。
- release-owning agent 将 successful publication 报告给 owner，source PR 不承载发布结果或状态评论。
...
- Given 运维手动触发 `workflow_dispatch(head_sha=<sha>, admin_action=release)`，When 更早的 backlog 里还存在 partial published target，Then workflow 拒绝越过缺口并保留该目标未发布状态。
...
- Given GitHub Release 创建/更新失败，When workflow 收尾，Then publication ledger 仍未记录该 target，queue 不会把它视为已发布。
- Given publish 全部成功，When workflow 完成，Then publication ledger 已记录，且 release-owning agent 向 owner 报告 successful publication；source PR 不新增发布结果或状态评论。
