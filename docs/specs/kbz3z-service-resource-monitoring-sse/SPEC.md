...
- 服务详情页由页面级资源监控 controller 统一加载历史、管理可见性和维护唯一一条资源 SSE；顶部摘要与监控面板消费同一份可见态样本，REST 历史响应不得覆盖其之后到达的 SSE 样本。从其它详情子页重新激活监控标签时，页面必须在可见、在线、非只读且监控启用的前提下后台重拉当前窗口历史并替换资源 SSE 会话；不得并行保留旧会话。
- 监控面板可以继续将 REST 历史与 1s SSE 合并为显示样本，但历史采样缺口只能依据当前窗口内成功 REST 响应中的持久化样本判定；实时 SSE 样本不得改变历史 cadence、补写历史样本集合或制造缺口告警。
- 所有服务详情子页（包括概览）都保留顶部实时摘要；监控面板的 `3m`、`1h`、`24h` 短窗口叠加页面打开后的 `1s` SSE 实时点，`7d` 与 `30d` 只展示桶均值主线与对齐峰值提示，不能把实时原始点混入聚合序列。
...
- SSE：页面可见且在线、监控启用时由页面级 controller 订阅一条资源流，断线退避重连（1s→2s→5s）；隐藏页关闭连接，回前台以新的 snapshot 恢复。从其它详情子页进入监控标签也必须以新的 snapshot 替换资源会话，且任意时刻只保留一条活跃资源流。离线、只读快照和监控关闭态不创建连接。
...
- 服务详情页在开启状态可看到历史曲线与实时滚动，且所有详情子页顶部摘要都随同一条 SSE 更新；实时状态无需查看 footer 即可感知。
- 服务详情从其它子页切入监控标签时，必须重新读取当前窗口历史并建立新的资源 SSE，会话替换期间保留已有可见样本且不得产生重复活跃连接。
