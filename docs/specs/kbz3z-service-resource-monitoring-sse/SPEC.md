...
- 服务详情页由页面级资源监控 controller 统一加载历史、管理可见性和维护唯一一条资源 SSE；顶部摘要与监控面板消费同一份可见态样本，REST 历史响应不得覆盖其之后到达的 SSE 样本。
...
- SSE：页面可见且在线、监控启用时由页面级 controller 订阅一条资源流，断线退避重连（1s→2s→5s）；隐藏页关闭连接，回前台以新的 snapshot 恢复。离线、只读快照和监控关闭态不创建连接。
