...
- 桌面服务顶栏中的服务名与可见监控指标使用同一固定间距，不再由自动外边距拆开；窄屏继续隐藏指标组并保留服务名与顶部操作的既有布局。
...
- `版本` 子页已改为按组件实际可用宽度响应：版本区不足 `1080px` 时不挂载 `220px` 目录；卡片只保留既有两种形态，版本区达到 `730px` 时使用元信息 / 正文 / 固定 `11.5rem` 状态动作轨道宽卡，低于该值时复用既有单列窄卡及底部动作区。目录和卡片形态的选择不再依赖浏览器视口，因此主导航与服务导航展开、折叠时都不会把正文压成异常窄列。宽度观测优先使用 `ResizeObserver`；兼容回退只监听 `.appShell` 的 `class/style` 与窗口 resize，并合并到下一帧，目录显隐后会重新测量虚拟列表。
...
- 已补齐版本页 Storybook `play` 覆盖：目录/正文双虚拟化、当前版本初始居中、目录点击联动、尾部分页、仓库级图标入口、宽屏固定动作栏、中间桌面宽度隐藏目录并保持正文可读，以及移动端无目录/无横向溢出。
