...
- 日志时间列已固定为时间第一行、日期第二行；表头与正文行通过日志终端共享的 `18px`/`14px` 水平边距令牌对齐，不再依赖 OverlayScrollbars 内部 viewport 节点。时间轨道收窄为桌面 `128px`、窄屏 `112px`，避免两行时间信息右侧留下过量空白。桌面、`393x852` 移动端、Human/Raw 与 UTC 切换均由 Storybook 与浏览器断言覆盖。
- 时间列布局证据使用独立的移动端 render story，避免继承会改变筛选模式的异步 play；跟随最新回归通过当前 DOM 节点触发跳转，确保虚拟列表重排时仍验证真实吸底行为。
