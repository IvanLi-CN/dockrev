...
- 日志时间块桌面保持时间第一行、日期第二行；桌面表头与正文行通过日志终端共享的 `18px` 水平边距令牌对齐，不再依赖 OverlayScrollbars 内部 viewport 节点，时间轨道为 `128px`。窄屏使用 `14px` 水平边距并隐藏终端表头，日志行改为单行时间/等级首行与正文跨列次行，行内隐藏重复日期，并在首条有效记录及换日处渲染一次居中日期分隔线，不使用固定时间列轨道。桌面、真实 `393x852` 移动端、Human/Raw 与 UTC 切换均由独立 Storybook 场景与浏览器断言覆盖；日期边界 mock 还覆盖无效时间戳跳过、同日不重复分隔线和 UTC 跨日分隔线，UTC 断言会按时间戳标题计算期望的时间/日期文本与日期分隔线文本，而不是只检查按钮状态。
