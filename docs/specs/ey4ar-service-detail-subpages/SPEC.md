...
- 本 spec 不定义后端 API、DB schema、SSE 语义、update/rollback job 模型或权限控制；版本列表指定版本部署由 [`selected-version-service-updates`](../selected-version-service-updates/SPEC.md) 定义。
...
- `版本` 子页必须对比当前部署版本、candidate 与既有 rollback target，展示状态徽标与动作区。普通服务的较新版本通过 [`selected-version-service-updates`](../selected-version-service-updates/SPEC.md) 分类为普通更新或强制更新；Dockrev candidate 卡继续使用 `升级 Dockrev` 并复用顶部 supervisor 自我升级入口，其它更高版本只保留禁用动作位并明确解释“当前只能通过 supervisor 进入现有 candidate 对应的自我升级流程”。若 supervisor 自我升级入口本身处于 offline / checking / busy 等不可用状态，则所有 Dockrev 版本卡优先直接暴露该阻断原因，不再继续引导用户访问不可用入口。
...
- `版本` 子页在同一服务已有 update/rollback 任务提交中、执行中，或 rollback target 刷新中时，必须锁定不属于当前活动任务的版本动作。普通服务当前可操作的所选较新版本卡动作必须同步 update 阶段：提交时显示 `提交中` 且按钮不可点击，Job 建立后按状态显示 `排队中 / 更新中`，按钮保持加载态并可直接进入对应任务详情；顶部更新动作继续提供同一任务入口。Dockrev candidate 目录 chip 与 candidate 卡继续同步 supervisor 自我升级状态。版本页不得额外渲染独立的活动任务横幅或横幅式“查看任务”入口。
...
- `日志` 时间块在桌面有效记录中按“时间第一行、日期第二行”展示；正文时间块的左右内边距必须与表头一致，桌面使用 `18px` 并保持 `128px` 时间轨道，且该对齐不得依赖滚动组件的内部 DOM 结构。窄屏使用 `14px` 内边距并隐藏终端表头，日志行必须保持两行流式结构：第一行只放单行时间与日志等级，第二行是跨满正文宽度的日志内容，不使用桌面表格列轨道。普通日志之间使用低对比度分界线；日期分隔组件前不得再叠加普通行分界线。窄屏日期不得在每条记录中重复占一行；每个有效日期首次出现时，才在记录前显示一次居中的日期分隔线，日志顺序回跳到已显示日期时不重复插入。无效时间戳继续保留原始值单行回退。
...
- `日志` 必须支持默认吸底、用户上滚后暂停跟随、`跳到最新` 恢复吸底，以及查询非空时继续接收流但不自动跳底。自动跟随必须同时对日志数量和末条日志身份变化生效；因此会话缓冲已满、淘汰最旧行后数量保持不变时，新增日志仍必须继续贴底。虚拟列表必须完成新末条测量后再定位，避免多行日志只露出开头。
...
- Storybook 必须提供七子页稳定入口，并至少覆盖：旧链接默认概览、tabs active/order state、版本页锚点定位/虚拟渲染/宽窄卡布局/动作守卫、更新记录深链/混合排序/备份列命中与空占位/分页边界/更新日志定位/空态/行级跳转、备份页记录列表/空态、日志深链、日志普通追加与 2,000 行满缓冲淘汰后的继续跟随、设置抽屉入口或监控页稳定渲染，以及移动端更新记录与版本卡无横向滚动。
...
- 日志页桌面保持时间戳 / 等级 / 正文三列的可读性；移动端使用单行时间 + 等级首行、正文跨列次行的流式布局，并以日期变化分隔线提供日期上下文，必要时收紧间距而不是回退到终端模拟器。
...
- 用户访问 `.../logs`：
  ...
  - 若用户未主动离开底部且查询为空，则新日志到达后自动保持在最新位置；该规则同时覆盖普通追加和满缓冲淘汰最旧行后末条替换的情况。
- 用户访问 `.../settings`：
  - 共享 hero/banner/top actions 与普通服务详情一致。
...
- Given 普通服务的 update 已从一个可操作的较新版本卡发起
  When 任务从提交态进入 queued/running
  Then 所选版本卡按钮依次显示 `提交中 / 排队中 / 更新中`；提交态按钮不可点击，Job 建立后的加载按钮可直接进入对应任务详情，其他版本动作保持锁定，且版本内容区不出现独立活动任务横幅。
...
- Given 服务详情页处于 `日志`、缓冲已满 2,000 行，且用户已通过 `跳到最新` 恢复跟随
  When SSE 到达一条多行新日志并淘汰最旧行
  Then 视口在新末条完成虚拟测量后仍位于底部，且不重新显示 `跳到最新` 控件。
...
- Given 服务详情页处于 `日志` 且存在可解析时间戳
  ...
  Then 桌面时间块显示时间第一行、日期第二行，正文时间块左缘与“时间”表头对齐，内边距为 `18px` 且时间轨道为 `128px`；窄屏内边距为 `14px`、隐藏终端表头，第一行仅并列单行时间与等级，第二行由日志内容跨列占满，每个有效日期只在首次出现时以居中分隔线显示一次，回跳到已显示日期时不重复插入。
...
- Given 服务详情 stories 已更新
  ...
  Then 至少能验证旧链接默认概览、tabs active/顺序切换行为、更新记录深链/混合列表/备份摘要命中与空占位/空态/行级跳转、备份页与日志页的核心入口稳定可用、桌面日志三列对齐、`393x852` 移动端日志表头隐藏、时间/等级首行保持单行、正文次行跨列且每个有效日期的分隔线只出现一次、移动端更新记录无横向滚动，以及设置页或监控页稳定渲染；时间布局证据故事必须独立渲染，不继承会异步改变显示模式的交互。
...
- `docs/specs/selected-version-service-updates/SPEC.md`
