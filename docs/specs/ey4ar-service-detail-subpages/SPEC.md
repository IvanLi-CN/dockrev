...
- `日志` 时间块在桌面有效记录中按“时间第一行、日期第二行”展示；正文时间块的左右内边距必须与表头一致，桌面使用 `18px` 并保持 `128px` 时间轨道，且该对齐不得依赖滚动组件的内部 DOM 结构。窄屏使用 `14px` 内边距并隐藏终端表头，日志行必须保持两行流式结构：第一行只放单行时间与日志等级，第二行是跨满正文宽度的日志内容，不使用桌面表格列轨道。窄屏日期不得在每条记录中重复占一行；首条有效记录及每次日期变化时，才在记录前显示一次居中的日期分隔线。无效时间戳继续保留原始值单行回退。
