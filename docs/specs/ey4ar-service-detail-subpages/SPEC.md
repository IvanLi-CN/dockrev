...
- `日志` 时间块必须在每条有效记录中按“时间第一行、日期第二行”展示；正文时间块的左右内边距必须与表头一致，桌面使用 `18px` 并保持 `128px` 时间轨道，且该对齐不得依赖滚动组件的内部 DOM 结构。窄屏使用 `14px` 内边距并隐藏终端表头，日志行改为两行流式布局：第一行是时间块与日志等级，第二行是跨满正文宽度的日志内容，不使用桌面表格列轨道。
...
- 日志页桌面保持时间戳 / 等级 / 正文三列的可读性；移动端使用时间块 + 等级首行、正文跨列次行的流式布局，必要时收紧间距而不是回退到终端模拟器。
...
- Given 服务详情页处于 `日志` 且存在可解析时间戳
  ...
  Then 时间块显示时间第一行、日期第二行；桌面正文时间块左缘与“时间”表头对齐，内边距为 `18px` 且时间轨道为 `128px`；窄屏内边距为 `14px`、隐藏终端表头，第一行并列时间块与等级，第二行由日志内容跨列占满。
...
- Given 服务详情 stories 已更新
  ...
  Then 至少能验证旧链接默认概览、tabs active/顺序切换行为、更新记录深链/混合列表/备份摘要命中与空占位/空态/行级跳转、备份页与日志页的核心入口稳定可用、桌面日志三列对齐、`393x852` 移动端日志表头隐藏且时间/等级首行与正文次行跨列、移动端更新记录无横向滚动，以及设置页或监控页稳定渲染；时间布局证据故事必须独立渲染，不继承会异步改变显示模式的交互。
