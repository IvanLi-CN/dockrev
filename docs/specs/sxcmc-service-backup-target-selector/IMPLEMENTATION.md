...
- Catalog note: fast-track（service backup target API + compose-derived candidates + Service detail drawer UI + Storybook evidence）
...
- 已更新 Service Detail 的“服务保护设置”抽屉，支持 `Volumes` / `Bind paths` 直选、三策略按钮组（`不备份 / 停机备份 / 在线备份`）、共享提示、空态文案与只读备份说明区块。
- 已更新 Storybook mock API 与页面故事，覆盖有候选、共享关闭、空态与只读说明等关键状态。

## Remaining Gaps

- `bun run test-storybook` 在本轮环境中出现脚本收尾超时噪音；需结合已通过的 story assertions 与独立视觉证据一起解读，不影响已生成的页面状态覆盖。
...
- Web: client types、ServiceDetail state/source、服务保护抽屉 UI、样式、Storybook mock handlers 与页面故事。
