...
- 已实现 Service Detail 与 Stack Detail 的最终策略结果摘要、最近三次更新记录，以及独立响应式设置抽屉；桌面从右侧出现，移动端从底部出现。Services/Operations 更新候选列表不再展示 Stack 策略快捷按钮，Service Detail 顶部保留 Stack 详情入口。
- 已实现非线性滑块、规则预览、抽屉 Storybook 覆盖与视觉证据落盘；PR 复用截图仍需主人明确提交授权。
...
- Web: Service/Stack policy editor drawer, detail summary cards, recent update records, Stack detail page, Service Detail stack navigation, client types, mock API, Storybook states.
