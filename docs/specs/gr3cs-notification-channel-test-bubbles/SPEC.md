...
- 成功/失败气泡支持点击气泡外部关闭；且最少展示 3 秒（3 秒内点过外部则到 3 秒时消失；不点外部不自动消失）。
...
- 成功/失败气泡：点击气泡外部可关闭；出现后三秒内不应消失，若三秒内点过外部则到三秒时自动消失；不点外部不自动消失。
- 失败气泡包含具体错误信息（非泛化错误）。
...
- 2026-03-05: 成功/失败气泡增加外部点击关闭；最少展示 3 秒且不自动消失。
