...
- `Review Policy` 必须使用 trusted-source 本地 required-check 语义：PR 元数据路径使用 `pull_request_target` 触发，配合 `pull_request_review` + `merge_group` 重新评估，结论由 dedicated local required check `Review Policy Gate` 直接表达。
...
- `PR Label Gate` 必须使用 trusted-source workflow-backed gate：PR 元数据路径使用 `pull_request_target` 触发，并支持 `merge_group` 对入口 PR 验证 `type:*` 与 `channel:*` 标签契约。
...
- repo 内 workflow、静态 contract-check 与 `quality-gates` 声明必须能被当前 topic validator 一致识别，不允许“仓库自认为完成、topic 仍判 drift”；metadata-only gate 也不得运行 PR 自改 workflow 逻辑。
...
- 若 merge queue 的 commit-associated pulls 未包含 `head_ref` 中可证明的入口 PR 编号，必须视为异常并失败；额外 associated pulls 不得自动视为当前 queue 批次成员。
...
- Given 一个 merge queue 组，When 入口 PR 未满足条件 review，Then `Review Policy Gate` 失败并标出具体 PR。
- Given 仓库内 workflow、topic validator 与静态 contract-check，When 校验 trusted-source / merge queue / required-check / 条件 review 语义，Then 不再出现 PR 自改 gate、validator drift 或 main/PR check context 冲突。
...
- [x] M1: `Review Policy` 对齐 trusted-source topic 契约，补齐 merge queue fail-closed 评估。
- [x] M2: `PR Label Gate` 支持 merge queue 入口 PR 校验，并对齐 trusted-source 的 PR 路径语义。
...
- 2026-03-11: 根据 review 结果修正 topic 契约：PR 元数据 gate 回到 trusted-source `pull_request_target`，merge queue 只要求 `head_ref` 证明入口 PR，忽略无法证明属于当前批次的额外 associated pulls，owner/maintainer 真正免 review。
