...
- migration `0024_normalize_auto_update_digest_identity` also consolidates candidate rows before writing canonical digests: it chooses a deterministic keeper, rebinds pending rows and existing `superseded_by_candidate_id` history references to that keeper, cancels or stop-controls duplicate candidate actions that have no retained active pending row, and removes duplicate candidate facts so the existing unique key represents canonical identity.
