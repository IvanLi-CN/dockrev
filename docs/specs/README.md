...
| taauj | Dockrev：Release Tag 预创建与发布完成态合同 | 部分完成（3/4） | `taauj-release-api-tag-publish-contract/SPEC.md` | 2026-04-01 | follow-up（explicit tag push restored; release API only updates assets/release; publication ledger still waits for release+comment success） |
...
| r8x2v | Dockrev：Stable Release 自动消费生产部署 | 已完成 | `r8x2v-release-stable-production-auto-deploy/SPEC.md` | 2026-04-03 | fast-track（release 成功后可选自动 pull/up 生产 latest，并回验公网版本） |
