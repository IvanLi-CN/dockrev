...
| taauj | Dockrev：Release Tag 预创建与发布完成态合同 | 部分完成（3/4） | `taauj-release-api-tag-publish-contract/SPEC.md` | 2026-04-05 | follow-up（explicit tag push restored; release API only updates assets/release; historical tag-backed backlog now auto-reconciles publication ledger before queue selection; remote release recovery still pending） |
