...
| dk4dd | Dockrev：GHCR Webhook 注册维护专页（Settings 预览 + 专页维护） | 已实现，待 PR 收敛 | `dk4dd-ghcr-webhook-registry-maintenance/SPEC.md` | 2026-03-02 | fast-track（服务端筛选分页与有界刷新） |
...
| 8xt2t | Dockrev：运行态版本漂移自动发现（runtime diff scan + SSE） | 已完成 | `8xt2t-runtime-drift-scan-sse/SPEC.md` | 2026-06-09 | migrated from legacy plan; moving-tag runtime image ID fallback |
| 5umc8 | Dockrev：发现次数气泡改为版本时间线 | 已完成 | `5umc8-discovery-count-timeline/SPEC.md` | 2026-04-10 | fast-track（runtime startedAt 真相源 + discovery timeline lazy API + shared hover-pin popover；PR #176 restores count/timeline parity for repeated unresolved alias history and live alias dedupe；2026-03-31 修复紧凑 badge 锚点回归并刷新视觉证据；2026-04-10 回滚 release-drawer 引入的双 trigger 回归，恢复单一可见标记） |
...
| qnq3w | Dockrev：Release Publication 对齐 latest 与 publication ledger | 已完成 | `qnq3w-release-publication-latest-pr-comment/SPEC.md` | 2026-03-28 | fast-track（publication ledger + latest pointer + self-test/README sync；source-PR release comment 子合同已废止） |
...
| htwyp | Dockrev：Cleanup 顶部按钮图标与估算兜底收口 | 已完成 | `htwyp-cleanup-estimate-fallback-and-topbar-icons/SPEC.md` | 2026-04-07 | fast-track（PR #205 released as 0.39.3；2026-04-07 follow-up hardens runtime estimate fallbacks so Cleanup no longer stalls on unknown sizes when daemon metadata is missing） |
