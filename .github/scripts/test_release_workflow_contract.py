...
assert "checks: read" in preparation
assert "checks: read" in completion
assert "ref: ${{ github.event.workflow_run.pull_requests[0].base.sha || github.sha }}" in preparation
...
assert "group: ${{ needs.identity.outputs.channel == 'stable' && 'release-publish-stable' || format('release-publish-{0}', needs.identity.outputs.merge_sha) }}" in release
assert "cancel-in-progress: false" in release
assert "release-latest-lock" not in release
assert "Release-Latest-Lock-" not in release
