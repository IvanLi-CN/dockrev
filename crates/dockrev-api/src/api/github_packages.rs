...
async fn emit_github_packages_delivery_event(state: &Arc<AppState>, delivery_id: &str) {
...
#[serde(rename_all = "camelCase")]
pub(super) struct GitHubPackagesWebhookDeliveryEventsQuery {
    #[serde(default)]
    after_id: i64,
}

pub(super) async fn github_packages_webhook_delivery_events(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
...
pub(super) async fn list_github_packages_webhook_deliveries(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
...
pub(super) async fn sync_github_packages_webhooks(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
