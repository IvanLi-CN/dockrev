...
fn ensure_settings_resource_monitor_columns(conn: &rusqlite::Connection) -> anyhow::Result<()> {
    ...
    let desired = [
        ...
        Col {
            ...
            ddl: "ALTER TABLE settings ADD COLUMN resource_sample_interval_seconds INTEGER NOT NULL DEFAULT 5",
        },
    ];
    ...
    conn.execute(
        r#"
UPDATE settings
SET resource_sample_interval_seconds = 5
WHERE resource_sample_interval_seconds NOT IN (5, 10, 30, 60, 300)
"#,
        [],
    )?;
...
pub(super) fn migrate(conn: &mut rusqlite::Connection) -> anyhow::Result<()> {
    ...
    apply_migration_0013_job_history_retention(conn)?;
    auto_archive_missing_discovery_projects_on_startup(conn)?;
    Ok(())
}

fn apply_migration_0013_job_history_retention(
    ...
    // SQLite cannot alter a foreign-key action in place. Preserve every backup row
    // while replacing the job reference with a nullable SET NULL relationship.
    tx.execute_batch(
        r#"
...
CREATE INDEX idx_job_service_targets_service_job
  ON job_service_targets(service_id, job_id);
...
CREATE INDEX idx_jobs_terminal_finished_at
