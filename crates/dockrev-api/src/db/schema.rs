...
SELECT
  ...
  CASE
    WHEN LOWER(j.reason) = 'schedule' THEN 'schedule'
    ELSE 'github_webhook'
  END,
...
"#,
    ...
    // Preserve historical strict SemVer evidence only when it is still parseable. A floating tag
    // must remain awaiting inference, while a previously resolved display tag can become ready.
